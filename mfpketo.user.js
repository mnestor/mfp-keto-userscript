...
function main() {
	...
	food_tr_elements.each(function () {
		...
		if (delete_td.hasClass('delete')) {
			...
			if ((carbs - fiber) < 0) {
				tds.eq(carbs_i).text(fiber);
				tds.eq(net_carbs_i).text(0);
			}
		}
	});
	
	var bottom_tr_elements = jQuery('.food_container tr.bottom, .food_container tr.total').not('.remaining');
	bottom_tr_elements.each(function () {
		...
		if (!jQuery(this).hasClass('alt')) {
			net_carbs = 0;
			var total_col = jQuery(this)
			if (jQuery(this).hasClass('total')) {
				total_col = total_col.prevAll().not('.bottom').not('.meal_header').not('.spacer');
			} else {
				total_col = total_col.prevUntil('.meal_header');
			}
			total_col.each(function(){ net_carbs += parseInt(jQuery(this).find('td').eq(net_carbs_i).text()); });
			if (!isNaN(net_carbs)) {
				if (net_carbs < 0) net_carbs = 0;
				tds.eq(net_carbs_i).text(net_carbs);
			} else if (jQuery(this).hasClass("total")) {
				tds.eq(net_carbs_i).text("090");
			}
		} else {
			// record goal
